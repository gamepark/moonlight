...
rules/src/                    # Server-side game logic
  ...
  ├── MoonlightRules.ts       # Main rules class
  ├── MoonlightSetup.ts       # Initial game setup
  └── MoonlightOptions.ts     # Game configuration
...
Translation files are located in `app/public/translation/` (one JSON file per language: `en.json`, `fr.json`). The project uses `i18next` + `react-i18next`.
